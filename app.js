...
function nextToPassword() {
    ...
    if (!userData.name.trim()) { alert("Please enter your name to proceed."); return; }
...
function checkSpam() {
    ...
    if (num.startsWith('91') || num.length % 2 === 0) {
        resultBox.style.background = "rgba(220,38,38,0.2)"; resultBox.style.border = "1px solid #dc2626"; resultBox.style.color = "#f87171"; resultBox.innerText = "🚨 HIGH SPAM RISK!";
    } else {
        resultBox.style.background = "rgba(16,185,129,0.2)"; resultBox.style.border = "1px solid #10b981"; resultBox.style.color = "#34d399"; resultBox.innerText = "✅ SAFE NUMBER";
    }
}
...
function nextToQuiz() { 
    ...
    quizQuestions.forEach((currentQ, qIdx) => {
        ...
        qBlock.style.marginBottom = "20px"; qBlock.style.borderBottom = "1px solid #334155"; qBlock.style.paddingBottom = "15px";
        
        const qTitle = document.createElement('div');
        qTitle.style.background = "#0f172a"; qTitle.style.padding = "10px"; qTitle.style.borderRadius = "8px"; qTitle.style.fontSize = "13px"; qTitle.style.marginBottom = "10px"; qTitle.style.fontWeight = "600";
        qTitle.innerText = `${qIdx + 1}. ${currentQ.q}`;
        qBlock.appendChild(qTitle);
        
        const optionsDiv = document.createElement('div');
        optionsDiv.style.display = "flex"; optionsDiv.style.flexDirection = "column"; optionsDiv.style.gap = "6px";
        
        currentQ.o.forEach((opt, oIdx) => {
            const label = document.createElement('label');
            label.style.display = "flex"; label.style.alignItems = "center"; label.style.gap = "8px"; label.style.background = "#1e293b"; label.style.padding = "10px"; label.style.borderRadius = "6px"; label.style.cursor = "pointer"; label.style.fontSize = "12px";
            
            const radio = document.createElement('input');
            radio.type = "radio"; radio.name = `question_${qIdx}`; radio.value = oIdx;
            
            label.appendChild(radio); label.appendChild(document.createTextNode(opt)); optionsDiv.appendChild(label);
        });
        qBlock.appendChild(optionsDiv); container.appendChild(qBlock);
    });
}

function submitFinalData() {
    let calculatedScore = 0; let allAnswered = true;
    quizQuestions.forEach((currentQ, qIdx) => {
        const selected = document.querySelector(`input[name="question_${qIdx}"]:checked`);
        if (!selected) { allAnswered = false; }
        else if (parseInt(selected.value) === currentQ.a) { calculatedScore++; }
    });
    
    if (!allAnswered) { alert("Please answer all 15 questions before submitting!"); return; }
    userData.score = calculatedScore;

    // Local storage safe buffer to prevent cross crash
    let offlineRecords = JSON.parse(localStorage.getItem("cyber_offline_db")) || [];
    offlineRecords.push({
        name: userData.name, role: userData.role, passwordStrength: userData.passwordStrength, spamNumber: userData.spamNumber, score: `${userData.score}/${quizQuestions.length}`
    });
    localStorage.setItem("cyber_offline_db", JSON.stringify(offlineRecords));

    if (db) {
        db.collection("cyber_hygiene_records").add({
            name: userData.name, role: userData.role, passwordStrength: userData.passwordStrength, spamNumber: userData.spamNumber, score: `${userData.score}/${quizQuestions.length}`, timestamp: firebase.firestore.FieldValue.serverTimestamp()
        })
        .then(() => {
...
function fetchLiveRecords() {
    ...
    const appendGridRow = (data) => {
        const tr = document.createElement('tr');
        let bg = data.passwordStrength === 'Strong' ? '#064e3b' : (data.passwordStrength === 'Medium' ? '#78350f' : '#7f1d1d');
        let tc = data.passwordStrength === 'Strong' ? '#34d399' : (data.passwordStrength === 'Medium' ? '#fbbf24' : '#f87171');
